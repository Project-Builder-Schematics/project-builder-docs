...
| [`modify`](#modify--the-structural-mutation) | A structural, AST-aware edit — reached **through dialects** | dialect handle | Target must exist and parse |
...
The eighth mutation is the one that doesn't come from `@pbuilder/sdk/commons` — and that's
...
into an AST. That understanding lives in the [dialects](/guides/typescript-and-react/):
...
Whichever way you author it — a named op like `addImport` or the `.modify()` escape hatch —
...
See [TypeScript & React Dialects](/guides/typescript-and-react/) for the full authoring
surface.
